locals {
  ...
  antivirus_queue                     = "arn:aws:sqs:${var.region}:${data.aws_caller_identity.current.account_id}:${project}-antivirus-${local.environment}"
  antivirus_queue_url                 = "https://sqs.${var.region}.amazonaws.com/${data.aws_caller_identity.current.account_id}/${project}-antivirus-${local.environment}"
  checksum_queue                      = "arn:aws:sqs:${var.region}:${data.aws_caller_identity.current.account_id}:${project}-checksum-${local.environment}"
  checksum_queue_url                  = "https://sqs.${var.region}.amazonaws.com/${data.aws_caller_identity.current.account_id}/${project}-checksum-${local.environment}"
  download_files_queue                = "arn:aws:sqs:${var.region}:${data.aws_caller_identity.current.account_id}:${project}-download-files-${local.environment}"
  download_files_queue_url            = "https://sqs.${var.region}.amazonaws.com/${data.aws_caller_identity.current.account_id}/${project}-download-files-${local.environment}"
  file_format_queue                   = "arn:aws:sqs:${var.region}:${data.aws_caller_identity.current.account_id}:${project}-file-format-${local.environment}"
  file_format_queue_url               = "https://sqs.${var.region}.amazonaws.com/${data.aws_caller_identity.current.account_id}/${project}-file-format-${local.environment}"
