resource "aws_lambda_function" "lambda_function" {
  ...
  environment {
    variables = {
      ...
      SQS_URL     = "https://sqs.${var.region}.amazonaws.com/${data.aws_caller_identity.current.account_id}/tdr-api-update-${local.environment}"
    }
  }
}
...
resource "aws_iam_policy" "lambda_policy" {
  count  = local.count_av_yara
  policy = templatefile("${path.module}/templates/av_lambda.json.tpl", { environment = local.environment, account_id = data.aws_caller_identity.current.account_id, sqs_arn = aws_sqs_queue.lambda_failure_queue.*.arn[0] })
