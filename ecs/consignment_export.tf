...
resource "aws_ecs_task_definition" "consignment_export_task_definition" {
  count                    = local.count_consignment_export
  container_definitions    = templatefile("${path.module}/templates/consignment_export.json.tpl", { log_group_name = aws_cloudwatch_log_group.consignment_export_log_group[count.index].name, app_environment = local.environment, management_account = data.aws_ssm_parameter.mgmt_account_number.value })
