...
EOM
cat create-user | PGPASSWORD=${db_password} psql -h ${db_host} -U ${db_username} -d consignmentapi
cat <<\EOF >> connect.sh
...
aws sts assume-role --role-arn arn:aws:iam::${account_number}:role/BastionTestAssumeRole --role-session-name bastionrolesession > creds
